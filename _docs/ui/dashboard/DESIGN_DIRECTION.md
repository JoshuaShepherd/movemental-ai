...
### Pattern 3: Content Dashboard with Card Grid
- **Source(s)**: `gamma-dashboard-content-grid.png`
- **What it solves**: Displays user's content library in browsable, filterable grid
- **Implementation guidance**:
  - **Left sidebar**:
    - User/workspace switcher at top
    - Main navigation: Gammas, Shared with you, Sites, AI Images
    - Folders section with custom folder support
    - Templates, Themes, Custom fonts
    - Settings & members, Contact support, Share feedback
  - **Top bar**: "Create new" + "New gamma" + Import buttons
  - **Filter tabs**: All | Recently viewed | Created by you | Favorites
  - **Content grid**: 4-5 column card layout
    - Each card: Preview thumbnail, title, site badge
    - Shared/Private indicator
    - Author avatar + "Created by you"
    - Last viewed timestamp
    - Pin/favorite indicator
  - **Account dropdown**: User email, Account settings, Language, Sign out, Credits display

### Pattern 4: App Store/Integration Marketplace
- **Source(s)**: `gorgias-app-store-categories.png`
- **What it solves**: Organizes integrations and apps by category for discovery
- **Implementation guidance**:
  - **Left sidebar**: Detailed category navigation
    - Channels: Help Center, Phone numbers, Email, Voice, SMS, Chat
    - Apps: Installed apps, App store (active), HTTP integration
    - Account: Store Management, Users, Teams, Access management
    - Advanced: Audit logs, REST API, Email Import
    - Profile: Your profile, Notifications, Password & 2FA
  - **Main content**: Category-organized card grids
    - "Featured" section with 4 highlighted apps
    - Category sections: Chat, Phone, SMS
    - Each app card: Icon/logo, name, description
    - "View All →" links per category
  - **Top search**: "Search for an app"
  - Category filter list in main area: All categories, Featured, Chat, Phone, SMS, Social media, etc.

### Pattern 5: Developer Resources Grid
- **Source(s)**: `workos-developer-resources.png`
- **What it solves**: Organizes developer/technical resources by function
- **Implementation guidance**:
  - Section headers: "Events and webhooks", "Resources"
  - 3-column card grid
  - Each card: Icon + title + description
  - Icons are simple line-style
  - Resource types: Event types, Data syncing, Observability, API reference, Integrations, SDKs, Email delivery, Dashboard, Glossary, Custom domains, Changelog
  - Light purple/gray gradient background
  - Clean, minimal styling
  - Click navigates to resource
...
| `gamma-dashboard-content-grid.png` | Gamma | Content card grid, folder sidebar, filter tabs, workspace | P1 |
| `gorgias-app-store-categories.png` | Gorgias | App marketplace, category navigation, integration cards | P2 |
| `workos-developer-resources.png` | WorkOS | Developer resources grid, icon cards, documentation | P2 |
