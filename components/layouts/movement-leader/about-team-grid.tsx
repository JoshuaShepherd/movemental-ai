  ...
  return (
    <section className={cn("relative w-full", className)}>
      ...
      <div className="px-6 sm:px-12 lg:px-20 py-20 text-center bg-mvmt-surface-light">
        ...
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-6 max-w-3xl mx-auto text-mvmt-text-primary font-mvmt-heading">
          The people behind the movement
        </h1>
        <p className="text-base leading-relaxed max-w-2xl mx-auto text-mvmt-text-secondary">
      ...
      <div className="px-6 sm:px-12 lg:px-20 py-16 bg-mvmt-surface-light-muted">
        <h2 className="text-xl font-bold mb-10 text-center text-mvmt-text-primary font-mvmt-heading">
          Our Team
        </h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {teamMembers.map((member) => (
            <div
              key={member.initials}
              className="rounded-lg p-6 border bg-mvmt-surface-light border-mvmt-border-light"
            >
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold text-mvmt-on-dark-primary bg-mvmt-surface-dark">
                  {member.initials}
                </div>
                <div>
                  <p className="font-semibold text-mvmt-text-primary">{member.name}</p>
                  <p className="text-xs text-mvmt-accent">{member.role}</p>
                </div>
              </div>
      ...
      {/* Values */}
      <div className="px-6 sm:px-12 lg:px-20 py-16 bg-mvmt-surface-light">
        <h2 className="text-xl font-bold mb-10 text-center text-mvmt-text-primary font-mvmt-heading">
          Our Values
        </h2>
        <div className="grid sm:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {values.map((v) => (
            <div key={v.title}>
              <h3 className="text-lg font-bold mb-2 text-mvmt-text-primary">{v.title}</h3>
              <p className="text-sm leading-relaxed text-mvmt-text-secondary">{v.description}</p>
            </div>
          ))}
        </div>
      </div>
