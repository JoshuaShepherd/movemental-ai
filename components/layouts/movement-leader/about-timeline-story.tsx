...
export function AboutTimelineStory({ className }: AboutTimelineStoryProps) {
  return (
    <section className={cn("relative w-full min-h-screen", className)}>
      {/* Hero */}
      <div className="px-6 sm:px-12 lg:px-20 py-24 text-center bg-[var(--mvmt-surface-dark)]">
        <p className="text-xs font-bold tracking-widest uppercase mb-4 text-[var(--mvmt-accent)]">
          Our Story
        </p>
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-6 max-w-3xl mx-auto text-[var(--mvmt-on-dark-primary)] font-mvmt-heading">
          A decade of building movements
        </h1>
        <p className="text-base leading-relaxed max-w-2xl mx-auto text-[var(--mvmt-on-dark-secondary)]">
      ...
      <div className="px-6 sm:px-12 lg:px-20 py-20 bg-[var(--mvmt-surface-dark-elevated)]">
        <div className="max-w-3xl mx-auto relative">
          {/* Vertical line */}
          <div className="absolute left-6 sm:left-8 top-0 bottom-0 w-px bg-[var(--mvmt-border-dark)]" />
          ...
          <div className="space-y-12">
            {milestones.map((m, i) => (
              <div key={m.year} className="relative pl-16 sm:pl-20">
                {/* Dot */}
                <div
                  className="absolute left-4 sm:left-6 top-1 w-4 h-4 rounded-full border-2 border-[var(--mvmt-accent)]"
                  style={{
                    background: i === milestones.length - 1 ? "var(--mvmt-accent)" : "var(--mvmt-surface-dark-elevated)",
                  }}
                />
                <p className="text-xs font-bold tracking-widest uppercase mb-1 text-[var(--mvmt-accent)]">
                  {m.year}
                </p>
                <h3 className="text-lg font-bold mb-2 text-[var(--mvmt-on-dark-primary)]">
                  {m.title}
                </h3>
      ...
      {/* Bottom CTA */}
      <div className="px-6 sm:px-12 lg:px-20 py-12 text-center bg-[var(--mvmt-surface-dark)]">
        <p className="text-sm mb-4 text-[var(--mvmt-on-dark-secondary)]">
