...
export function AssessmentsMoodGradient({ className }: AssessmentsMoodGradientProps) {
  ...
  return (
    <section
      className={cn("relative w-full min-h-[70vh] md:min-h-[80vh] flex flex-col items-center justify-center overflow-hidden", className)}
      style={{ background: "var(--mvmt-gradient-hero-brand)" }}
    >
      {/* Decorative cloud shapes */}
      <div className="absolute top-16 right-20 w-20 h-8 rounded-full opacity-20" style={{ backgroundColor: "var(--mvmt-on-dark-primary)" }} />
      <div className="absolute top-24 right-32 w-12 h-5 rounded-full opacity-15" style={{ backgroundColor: "var(--mvmt-on-dark-primary)" }} />
      ...
      <div className="flex items-center gap-2 mb-10">
        {steps.map((step, i) => (
          <div key={step.label} className="flex items-center gap-2">
            ...
            {i < steps.length - 1 && (
              <span className="text-xs mx-1" style={{ color: "var(--mvmt-on-dark-muted)" }}>•</span>
            )}
          </div>
        ))}
      </div>
      ...
      <button
        ...
        style={{ backgroundColor: "var(--assess-close-bg)" }}
      >
        <span style={{ color: "var(--mvmt-on-dark-primary)" }}>✕</span>
      </button>

      {/* Content */}
      <p className="text-sm mb-2" style={{ color: "var(--mvmt-on-dark-secondary)", fontFamily: "var(--mvmt-font-body)" }}>
      ...
      <div className="flex gap-4">
        {moods.map((mood) => (
          <button
            key={mood.label}
            className="flex flex-col items-center justify-center w-24 h-24 sm:w-28 sm:h-28 rounded-xl cursor-pointer transition-transform hover:scale-105"
            style={{ backgroundColor: "var(--assess-mood-card-bg)" }}
          >
            <span className="text-3xl mb-2">{mood.icon}</span>
            <span className="text-xs font-medium" style={{ color: "var(--mvmt-on-dark-primary)" }}>{mood.label}</span>
          </button>
        ))}
      </div>
    </section>
