...
export function AuthBrandedHero({ className }: AuthBrandedHeroProps) {
  return (
    <section className={cn("relative w-full min-h-screen grid md:grid-cols-2", className)}>
      {/* Left: Brand Story */}
      <div className="flex flex-col justify-center px-8 sm:px-16 lg:px-20 py-16 bg-mvmt-surface-dark">
        <p className="text-xs font-bold tracking-widest uppercase mb-4 text-mvmt-accent">
          Movemental
        </p>
        <h2 className="text-3xl sm:text-4xl font-bold mb-6 leading-tight text-mvmt-on-dark-primary font-mvmt-heading">
          Tools built for the people building movements
        </h2>
        <p className="text-sm leading-relaxed mb-10 text-mvmt-on-dark-secondary">
        ...
        <div className="space-y-4">
          {["Personalized fit-check assessments", "Curated content from leading practitioners", "AI-powered research and insights"].map((item) => (
            <div key={item} className="flex items-center gap-3">
              <div className="w-5 h-5 rounded-full flex items-center justify-center text-2xs shrink-0 text-mvmt-cta-text bg-mvmt-accent">
                ✓
              </div>
              <p className="text-sm text-mvmt-on-dark-secondary">{item}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Right: Form */}
      <div className="flex flex-col justify-center px-8 sm:px-16 lg:px-20 py-16 bg-mvmt-surface-light">
        <div className="max-w-sm w-full mx-auto">
          ...
          <label className="block mb-4">
            <span className="text-xs font-medium mb-1.5 block text-mvmt-text-primary">Full name</span>
            <input
              type="text"
          ...
          {/* Email */}
          <label className="block mb-4">
            <span className="text-xs font-medium mb-1.5 block text-mvmt-text-primary">Email</span>
            <input
              type="email"
          ...
          <label className="block mb-6">
            <span className="text-xs font-medium mb-1.5 block text-mvmt-text-primary">Password</span>
            <input
              type="password"
              placeholder="••••••••"
              className="w-full px-3 py-2.5 text-sm rounded-md border outline-none transition-colors bg-mvmt-surface-light border-mvmt-border-light text-mvmt-text-primary placeholder:text-mvmt-text-muted focus:border-mvmt-accent"
          ...
          <button className="w-full py-2.5 text-sm font-medium rounded-md border transition-colors border-mvmt-border-light text-mvmt-text-primary hover:bg-mvmt-surface-light-muted">
            Continue with Google
          </button>

          <p className="text-xs mt-8 text-center text-mvmt-text-muted">
            Already have an account?{" "}
            <Link href="/login" className="text-mvmt-accent hover:underline">Sign in</Link>
          </p>
        </div>
      </div>
    </section>
