...
export function ChatPanelSidebar({ className }: ChatPanelSidebarProps) {
  ...
  return (
    <section className={cn("relative w-full min-h-screen flex", className)} style={{ backgroundColor: "var(--mvmt-surface-light)" }}>
      {/* Left sidebar (dark) */}
      <aside className="w-56 flex-shrink-0 flex flex-col px-3 py-4" style={{ backgroundColor: "var(--mvmt-surface-dark)" }}>
        <div className="flex items-center gap-2 px-2 mb-5">
          <div className="w-6 h-6 rounded" style={{ backgroundColor: "var(--mvmt-accent)" }} />
          <span className="text-sm font-semibold" style={{ color: "var(--mvmt-on-dark-primary)" }}>Sam&apos;s Workspace</span>
        </div>

        <nav className="space-y-0.5 mb-6">
          {sidebarNav.map((item) => (
            <div key={item.label} className="flex items-center gap-2.5 px-2 py-1.5 rounded-md text-sm cursor-pointer" style={{ color: "var(--mvmt-on-dark-secondary)" }}>
        ...
        <div className="mb-4">
          <span className="text-[10px] font-semibold uppercase tracking-wider px-2" style={{ color: "var(--mvmt-on-dark-tertiary)" }}>Favorites</span>
          <div className="mt-1 px-2 py-1 text-xs" style={{ color: "var(--mvmt-on-dark-secondary)" }}>Church Planting Playbook</div>
        </div>

        <div>
          <span className="text-[10px] font-semibold uppercase tracking-wider px-2" style={{ color: "var(--mvmt-on-dark-tertiary)" }}>Spaces</span>
          <div className="mt-1 space-y-0.5 text-xs" style={{ color: "var(--mvmt-on-dark-secondary)" }}>
      ...
      <main className="flex-1 px-8 py-8 overflow-y-auto">
        <h1 className="text-2xl font-bold mb-6" style={{ color: "var(--mvmt-text-primary)" }}>Good afternoon, Brad</h1>

        {/* Recents card */}
        <div className="border rounded-xl p-5 mb-6" style={{ borderColor: "var(--mvmt-border-light)" }}>
          <h2 className="text-sm font-semibold mb-3" style={{ color: "var(--mvmt-text-primary)" }}>Recents</h2>
          <div className="space-y-2">
            {recentItems.map((item) => (
              <div key={item.title} className="flex items-center justify-between py-1.5 text-sm">
                <span style={{ color: "var(--mvmt-text-secondary)" }}>{item.title}</span>
                <span className="text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: "var(--mvmt-surface-light-muted)", color: "var(--mvmt-text-tertiary)" }}>
                  {item.tag}
                </span>
              </div>
            ))}
        ...
        <div className="border rounded-xl p-5" style={{ borderColor: "var(--mvmt-border-light)" }}>
          <h2 className="text-sm font-semibold mb-3" style={{ color: "var(--mvmt-text-primary)" }}>My Work</h2>
          <div className="flex gap-4 mb-4 text-xs font-medium" style={{ color: "var(--mvmt-text-tertiary)" }}>
            <span style={{ color: "var(--mvmt-text-primary)", borderBottom: "2px solid var(--mvmt-accent)" }} className="pb-1">To Do</span>
            ...
            {tasks.map((task) => (
              <div key={task.title} className="flex items-center justify-between py-1.5 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span style={{ color: "var(--mvmt-text-secondary)" }}>{task.title}</span>
                </div>
      ...
      <aside className="w-80 flex-shrink-0 border-l flex flex-col" style={{ borderColor: "var(--mvmt-border-light)" }}>
        <div className="px-5 py-4 border-b flex items-center gap-2" style={{ borderColor: "var(--mvmt-border-light)" }}>
          <span className="text-sm" style={{ color: "var(--mvmt-accent)" }}>&#10024;</span>
          <span className="text-sm font-semibold" style={{ color: "var(--mvmt-text-primary)" }}>Movemental AI</span>
        </div>

        <div className="flex-1 px-5 py-5 space-y-6 overflow-y-auto">
          <h3 className="text-base font-semibold" style={{ color: "var(--mvmt-text-primary)" }}>Ask anything or write with AI</h3>

          <div className="rounded-lg p-3" style={{ backgroundColor: "var(--mvmt-surface-light-muted)" }}>
            <p className="text-xs font-medium mb-1" style={{ color: "var(--mvmt-text-primary)" }}>Enhance Movemental AI&apos;s knowledge with Docs</p>
            <p className="text-[11px]" style={{ color: "var(--mvmt-text-tertiary)" }}>Connect your coaching documents, curriculum guides, and community playbooks.</p>
          </div>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {aiPrompts.map((prompt) => (
                <div key={prompt} className="px-3 py-2 text-xs rounded-lg border cursor-pointer" style={{ borderColor: "var(--mvmt-border-light)", color: "var(--mvmt-text-secondary)" }}>
                  {prompt}
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="px-3 py-2 text-xs rounded-lg border" style={{ borderColor: "var(--mvmt-border-light)", color: "var(--mvmt-text-secondary)" }}>
        ...
        <div className="px-4 py-3 border-t" style={{ borderColor: "var(--mvmt-border-light)" }}>
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg border" style={{ borderColor: "var(--mvmt-border-light)" }}>
            ...
            <span className="text-xs" style={{ color: "var(--mvmt-accent)" }}>&uarr;</span>
          </div>
        </div>
      </aside>
    </section>
