      ...
      <div className="container mx-auto px-6 sm:px-8 lg:px-12 py-16 md:py-24">
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          <div>
            ...
            <p className="mt-4 text-sm leading-relaxed max-w-md" style={{ color: "var(--mvmt-on-dark-muted)" }}>
            ...
            <div className="mt-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs" style={{ color: "var(--mvmt-on-dark-muted)" }}>Movement Catalyst &amp; Author</p>
              </div>
            </div>
          </div>
          ...
          >
            <p className="text-xs font-medium mb-1" style={{ color: "var(--mvmt-accent)" }}>
              Limited enrollment
            </p>
            <div className="flex items-baseline gap-3 mb-4">
              <h2 className="text-4xl font-bold" style={{ color: "var(--mvmt-text-primary)" }}>Free</h2>
              <span className="text-lg line-through" style={{ color: "var(--mvmt-text-muted)" }}>$450</span>
            </div>
            <p className="text-sm mb-1" style={{ color: "var(--mvmt-text-secondary)" }}>
              ✅ Starts March 2026 | 2 sessions per week
            </p>
            <div className="mt-6 space-y-3">
              <Link
                href="/fit-check"
                className="block w-full text-center px-6 py-3.5 text-sm font-semibold transition-colors"
                style={{
              ...
              <Link
                href="/about"
                className="block w-full text-center px-6 py-3.5 text-sm font-medium border transition-colors"
                style={{
                  borderColor: "var(--mvmt-border-light)",
        ...
        <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-6">
          {stats.map((s) => (
            <div key={s.label} className="text-center">
              <p className="text-3xl font-bold" style={{ color: "var(--mvmt-accent)" }}>{s.value}</p>
              <p className="text-xs uppercase tracking-wider mt-1" style={{ color: "var(--mvmt-on-dark-muted)" }}>{s.label}</p>
            </div>
          ))}
        </div>
      </div>
