...
const AccordionItem: React.FC<AccordionItemProps> = ({ id, question, answer }) => {
  ...
  return (
    <div className="group border-b border-border/60" id={id}>
      <button 
        onClick={() => setIsOpen(!isOpen)}
        aria-expanded={isOpen}
        className="flex w-full cursor-pointer items-center justify-between py-5 text-left text-foreground font-medium hover:text-primary transition-colors"
      >
        <span className="text-[1.0625rem]">{question}</span>
        <span className={cn("ml-6 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border transition-colors", isOpen ? "border-primary bg-primary text-primary-foreground" : "border-border group-hover:border-primary/50")}>
          <motion.svg 
...
const FAQ_GROUPS = [
  {
    ...
    items: [
      { slug: "what-first", q: "What should we do first?", a: <>Start with Safety. Reach out via our <Link to="/contact" className="text-primary hover:underline">Contact page</Link> to schedule a diagnostic call.</> },
      ...
      { slug: "what-is-it", q: "Is this a tool, a course, or a consulting engagement?", a: <>It is a consulting engagement mapped to a specific architecture. See <Link to="/work-with-us" className="text-primary hover:underline">Ways to Engage</Link>.</> },
  ...
  },
  {
    ...
    lede: "Safety, Sandbox…",
    items: [
      { slug: "what-is-path", q: "What is the Movemental AI Path?", a: <>It is the four-stage AI Stewardship Sequence: Safety, Sandbox, Skills, Solutions. Read it in the <Link to="/path" className="text-primary hover:underline">Path summary</Link>.</> },
      ...
      { slug: "safety-covers", q: "What does Safety actually cover?", a: <>Acceptable use policies, data boundary definitions, and theological red lines. See <Link to="/start-with-safety" className="text-primary hover:underline">Start with Safety</Link>.</> },
  ...
  },
  {
    ...
    items: [
      ...
      { slug: "movement-leaders-fit", q: "Where do movement leaders fit?", a: <>They are the relational glue guiding this transition. See our <Link to="/voices" className="text-primary hover:underline">Voices</Link> context.</> },
    ]
  },
  {
    id: "engagement",
    ...
    items: [
      { slug: "options", q: "What are the engagement options?", a: <>Safety Sessions, Guided Pathways, and Solutions Partnerships. See <Link to="/work-with-us" className="text-primary hover:underline">Work with Us</Link>.</> },
  ...
  },
  {
    ...
    items: [
      { slug: "voices-vs-evidence", q: "What is the difference between Voices and Evidence?", a: <>Evidence explains the problem structure; Voices are the actual practitioners validating it. See <Link to="/evidence" className="text-primary hover:underline">Evidence</Link>.</> },
      { slug: "case-studies", q: "Where are your case studies?", a: <>They are embedded in the audience path pages. See <Link to="/churches" className="text-primary hover:underline">Churches</Link> for an example.</> },
  ...
  },
  {
    ...
    items: [
      ...
      { slug: "after-safety", q: "What happens after Safety?", a: <>You are free to safely experiment inside structured Sandboxes. Read the <Link to="/field-guide" className="text-primary hover:underline">Field Guide</Link>.</> },
      ...
      { slug: "still-questions", q: "Where do we go if we still have questions?", a: <>Reach out through our <Link to="/contact" className="text-primary hover:underline">Contact page</Link>. We are here.</> },
    ]
  }
];
        ...
        <Container>
          <Reveal>
            ...
            <div className="hero-actions flex flex-wrap gap-4">
              ...
              <Link to="/path" className="btn-pill border border-inverse-border text-inverse-foreground hover:bg-inverse-foreground/10">Read the Path</Link>
            </div>
          </Reveal>
        </Container>
      </section>
        ...
        <Container width="narrow">
          <Reveal>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <Link to="/contact" className="btn-pill bg-[#FDFBF7] text-[#101828] hover:bg-[#E5E0D8]">Talk With Us</Link>
              <Link to="/start-with-safety" className="btn-pill border border-inverse-border text-inverse-foreground hover:bg-inverse-foreground/10">Start with Safety</Link>
            </div>
          </Reveal>
        </Container>
      </section>
