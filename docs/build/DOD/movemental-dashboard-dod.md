...
| DOD-02 | **MOU — access and legal sign:** Users who should sign can **open** the MOU from the dashboard, read it, and **complete a legally effective signature** (correct agreement version, signer identity captured, audit trail / storage as required). | In progress | Product: **Documents → Memorandum of Understanding (MOU)** in `authenticated-shell`; `/onboarding/agreement` renders `AgreementSigningPanel` with numbered steps, org context, DocuSign deep link when `NEXT_PUBLIC_DOCUSIGN_MOU_URL` is set, agreements register link, optional `NEXT_PUBLIC_DOCUSIGN_ENGAGEMENT_URL`. Webhook: `POST /api/webhooks/docusign` + `DOCUSIGN_CONNECT_HMAC_KEY`. **Remaining for “legally effective”:** counsel confirms envelope + version; production env has MOU URL; at least one executed sign recorded (DocuSign Connect or manual register). |
...
> **Production / legal (DOD-02):** In the live (or staging) environment, confirm `NEXT_PUBLIC_DOCUSIGN_MOU_URL` points at the current MOU envelope, open **Documents → MOU** as an authorized org user, complete signing in DocuSign, and verify the row appears in the signed agreements register (or Connect webhook path). Update DOD-02 to **Done** only after counsel confirms legal effectiveness. Append one row to Section 5 with environment, account class, and outcome.
...
| 2026-05-12 | AI / closed-loop pass | Repo review: `dashboard/page.tsx` auth redirect; `proxy.ts` `/dashboard` session; Documents → MOU → `AgreementSigningPanel` + env vars; `/sandboxlive/cohort/schedule` + `TrainingSchedulePanel` + Calendly URL resolution; `/assess` + `IntegrityDiagnosticForm` previously simulated submit | DOD-01 — DOD-04 | DOD-01 **Done** (code + stated stakeholder). DOD-02 **In progress** (product path shipped; legal + prod envelope verification outstanding). DOD-03 **In progress** (instructions in UI; live Calendly smoke outstanding). DOD-04 **Done** after wiring `IntegrityDiagnosticForm` → `POST /api/assess` with canonical 22 answers; `pnpm exec tsc --noEmit` clean |
