...
## 7. Sheet vs drawer — why a page change can look invisible

The screen **always stays mounted** when the drawer expands (invariant **I6**). The visitor literally cannot see sheet updates while the drawer is open.

What actually happens:

| Event | Drawer behavior | Can the visitor see the new sheet? |
| --- | --- | --- |
| LOCAL scene from **collapsed** dock (chip or typed regex) | Stays collapsed | **Yes** — immediate full-page swap |
| LOCAL scene while drawer was **already expanded** | **Auto-collapses** when `screenKey` changes | **Yes** — collapse reveals the new sheet |
| AGENT turn, reply **only** (no `ui_render`) | Expands (or stays expanded) | **No** — sheet unchanged behind scrim; only thread updates |
| AGENT turn **with** `ui_render` (e.g. `show_pricing`) | Expands for stream, then **auto-collapses** on `screenKey` change | **Yes** — collapse reveals the rendered screen |

So the “I typed something and nothing changed” report usually means one of:

1. The turn classified **AGENT**, the drawer expanded, and the host answered **in prose only** without calling a show tool — the home sheet is still there, hidden.
2. The turn classified **AGENT**, the drawer was already open, and any `ui_render` happened behind the scrim until the visitor collapses.
3. The visitor expected chat but hit a **high-confidence LOCAL** route — they should see a page swap with the dock collapsed; if the drawer was open, it should collapse to reveal it.

**Practical expectation on home:** Treat float chips and command-like first messages (*"pricing"*, *"contact us"*) as **navigation**. Treat unmatched, conversational first messages as **chat** (drawer expands, thread streams). After one agent reply, everything is chat until replay.
...
If the visitor expands and asks an open question, the host may pair a short thread reply with `ui_render` tools (`show_pricing`, `show_path`, etc.) per host prompt policy — speak **and** show.
