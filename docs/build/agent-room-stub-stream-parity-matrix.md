...
**Mode default:** `stream` (`src/lib/agent-room/mode.ts`)
...
| --- | --- | --- | --- |
| `SHOW home` | LOCAL | `run("opening")` → `show("home")` | Static `HomeScreen` on load (`ScreenState.opening`) |
| `WAIT 560` + `SAY` opening line | **LOCAL** | `SCENES.opening` via `playScene` | `OPENING_CHOREOGRAPHY` → `playOpeningChoreography` (`opening-choreography.ts`) |
| `GESTURE underline → #phrase` | **LOCAL** | same scene | same local module |
| `SUGGEST` chips (6) | LOCAL (stub) / static (stream) | `suggest()` binds `to` scene | `DEFAULT_SUGGESTIONS` in `composer.tsx`; routing per PAR-02 |
| Lead chip "Get a clear next AI step" | **LOCAL** → AGENT | `to: "beatIntro"` → `run("beatIntro")` | **PAR-02:** local `BEAT_INTRO_CHOREOGRAPHY`, then AGENT on "Okay, map it" |
| Leader portrait tap | AGENT | `leaderScene(i)` local | `onSay("Tell me about {name}")` (`stream-screen.tsx`) |
| REPLAY | **LOCAL** | `goHome()` → `run("opening")` | `reset()` → `playOpeningChoreography` |
