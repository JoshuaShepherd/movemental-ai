...
## Follow-on pack — stub ↔ stream parity (PAR)

INT-07 flipped the default to `"stream"` but left **composed-product gaps**: local opening choreography was missing until patched; stub `SCENES` and live agent remain parallel implementations; chip routing, beat→readback, and docs disagree on SSOT.

**Execute next:** [`../agent-room-stub-stream-parity/master_runner.md`](../agent-room-stub-stream-parity/master_runner.md) (PAR-00…PAR-07). **PAR-07** is the independent Claude reviewer session — final merge gate after PAR-06 draft sign-off.
