## Resend audit — movemental (movemental-ai) — 2026-08-17
...
Branch: `cursor/resend-email-audit-a596` (automation cloud branch). Re-applies unmerged safe fixes from draft PRs on `cursor/resend-email-audit-97cb` (2026-08-03) and `cursor/resend-email-audit-9217` (2026-08-10).

### MCP

| Server | Status |
|--------|--------|
| Resend | **needsAuth** — catalog `serverStatus: ready`, but `list-domains` / `list-api-keys` / `list-webhooks` / `list-emails` / `list-logs` / `list-templates` all returned `API key is invalid` (`validation_error`). No `mcp_auth` tool exposed for Resend in this environment. Re-auth required in Cursor Automations MCP settings. |
...
- Server-only send paths: Resend helpers under `src/lib/email/*` and `src/lib/book-email.ts` use `server-only` (or import guarded clients). Callers are API routes, cron services, and server libs — no Client Component Resend SDK usage found.
...
| **INFO** | Prior safe-fix draft PRs (#2, #3) still unmerged — this run re-applies the same hardening on a fresh cloud branch. |
...
- This report (`docs/build/temp/reports/resend-audit-20260817.md`)
...
- Merge or close superseded draft PRs (#2 / #3) when this PR lands
