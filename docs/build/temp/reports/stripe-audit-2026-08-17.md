## Stripe audit — movemental — 2026-08-17

**Tenant = movemental · Repo = JoshuaShepherd/movemental-ai · Stripe mode expected = live (prior MCP) / unknown (local env unset)**

Package `name`: `movemental`. No `src/lib/config/tenant.config.ts` (org hub — not brad-brisco / alan-hirsch). Branch: `cursor/stripe-config-and-functionality-7467`.

### MCP
- Stripe: **needsAuth** — server present; interactive `mcp_auth` unavailable in cloud agents. Live Stripe checks below cite **2026-08-03** MCP evidence (account `acct_1TJrhFA5zZEgTNPG`) and are marked stale pending re-auth. Environment setup action recorded to authenticate Stripe MCP for future runs.
...
- Fail-closed when unset: **PASS (501)** — checkout / PaymentIntent / webhook return `stripe_unconfigured` with HTTP **501** (re-applied this audit; `main` still had 503).
...
- Metadata: `type: "safestart"` + `enrollment_id` + `safety_plan` (re-applied; missing on `main`).
...
4. **HIGH — `amount_total` null coalesced to expected price (fixed this run)**  
   `main` still used `session.amount_total ?? SAFETY_ENROLLMENT_AMOUNT_CENTS`, which fail-opened when Stripe omitted `amount_total`. Now requires an exact match; also rejects non-`paid` `payment_status` when present. Prior harden PR #4 was open but not merged.
...
10. **INFO — Prior audit PRs still open**  
    #1 (`cursor/stripe-config-and-functionality-4276`) and #4 (`…-2183`) had overlapping env/metadata/501 hardenings not yet on `main`; re-applied on this branch.
...
- Merge/close superseded open Stripe audit PRs #1 / #4 after this lands.
