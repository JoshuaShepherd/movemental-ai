/**
 * Shared lesson browser for docs/html/course-previews/*.
 * Prerequisite: window.COURSE_LESSONS populated by fr-/gov-/cl-/ai-lessons-*.js
 * Optional: window.COURSE_PREVIEW_GROUP_LABELS, COURSE_PREVIEW_TITLE_SUFFIX,
 *            COURSE_PREVIEW_LESSON_FILES_HINT
 */
(function () {
  "use strict";

  var lessons = window.COURSE_LESSONS;
  var hint = window.COURSE_PREVIEW_LESSON_FILES_HINT || "lessons *.js";
  var titleSuffix = window.COURSE_PREVIEW_TITLE_SUFFIX || "Course preview";
  var groupLabels =
    window.COURSE_PREVIEW_GROUP_LABELS && typeof window.COURSE_PREVIEW_GROUP_LABELS === "object"
      ? window.COURSE_PREVIEW_GROUP_LABELS
      : {};
  ...
  if (!Array.isArray(lessons) || lessons.length === 0) {
    var rootMissing = document.getElementById("lesson-root");
    if (rootMissing) {
      rootMissing.innerHTML =
        "<p>Course content failed to load. Ensure all <code>" +
        escapeHtml(hint) +
        "</code> files are present.</p>";
    }
    return;
  }
  ...
  function render(id) {
    ...
    var header =
      ...
      "</h1>" +
      (L.meta ? '<p class="lesson-meta">' + escapeHtml(L.meta) + "</p>" : "") +
      "</header>";

    root.innerHTML = header + '<div class="lesson-body">' + (L.html || "") + "</div>";
    crumb.textContent = L.title;
    setActiveNav(id);
    document.title = (L.h1 || L.title) + " — " + titleSuffix;
    root.focus();
  }
