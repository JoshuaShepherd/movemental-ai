...
**Validation**: `pnpm contracts:check` → required `{"status":"LOCKED"}`  
**Status**: ✅ **LOCKED** — 215 entities (verified 2026-06-13)  
**Generate**: `pnpm generate:schemas` (`scripts/generate-zod-schemas.ts`)
...
### `src/lib/schemas/base.ts` - Base Schemas
...
### `src/lib/schemas/index.ts` - Entity Schemas
...
The `src/lib/schemas/index.ts` file also contains form-specific schemas for multi-step onboarding validation:
...
#### `src/lib/schemas/fit-check.ts` - Fit Check Assessment
...
#### `src/lib/schemas/onboarding-path.ts` - Onboarding Path
...
**Layer 1 Change** (`src/lib/db/schema.ts`):
```typescript
...
**Layer 2 Auto-Update** (`src/lib/schemas/index.ts`):
```typescript
