...
import { persistAffiliatesReview } from "@/lib/services/onboarding/onboarding-http.service";
...
export async function POST(request: NextRequest) {
  ...
  const affiliates =
    typeof body === "object" && body && "affiliates" in body
      ? (body as { affiliates?: unknown }).affiliates
      : undefined;

  if (!Array.isArray(affiliates)) {
    return NextResponse.json(
      { error: { code: "validation_error", message: "affiliates array is required." } },
      { status: 400 },
    );
  }
  ...
  const result = await persistAffiliatesReview({
