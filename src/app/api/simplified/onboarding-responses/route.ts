...
import { onboardingResponsesService } from "@/lib/services/simplified/onboarding-responses.service";
import { OnboardingResponsesInsertSchema, OnboardingResponsesUpdateSchema, OnboardingResponsesFiltersSchema } from "@/lib/schemas";
...
export async function GET(request: NextRequest) {
  ...
  const parsed = OnboardingResponsesFiltersSchema.safeParse({
    ...rawFilters,
  ...
  const result = await onboardingResponsesService.list(parsed.data);
  if (!result.success) {
...
export async function PATCH(request: NextRequest) {
  ...
  const parsed = OnboardingResponsesUpdateSchema.safeParse(updateData);
  if (!parsed.success) {
    return NextResponse.json(
      { error: { code: "VALIDATION_ERROR", message: parsed.error.message } },
      { status: 400 },
