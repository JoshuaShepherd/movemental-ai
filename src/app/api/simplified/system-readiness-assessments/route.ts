...
export async function GET(request: NextRequest) {
  ...
  const parsed = SystemReadinessAssessmentsFiltersSchema.safeParse({
    ...rawFilters,
  ...
  const result = await systemReadinessAssessmentsService.list(parsed.data);
  if (!result.success) {
...
export async function POST(request: NextRequest) {
  ...
  const parsed = SystemReadinessAssessmentsInsertSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      { error: { code: "VALIDATION_ERROR", message: parsed.error.message } },
      { status: 400 },
  ...
  const result = await systemReadinessAssessmentsService.create(parsed.data);
  if (!result.success) {
    return NextResponse.json({ error: result.error }, { status: 500 });
  }
...
export async function PATCH(request: NextRequest) {
  ...
  const parsed = SystemReadinessAssessmentsUpdateSchema.safeParse(updateData);
  if (!parsed.success) {
    return NextResponse.json(
      { error: { code: "VALIDATION_ERROR", message: parsed.error.message } },
      { status: 400 },
  ...
  const result = await systemReadinessAssessmentsService.update(id, parsed.data);
  if (!result.success) {
...
export async function DELETE(request: NextRequest) {
  ...
  const result = await systemReadinessAssessmentsService.delete(id);
  if (!result.success) {
    const status = result.error.code === "NOT_FOUND" ? 404 : 500;
    return NextResponse.json({ error: result.error }, { status });
  }
