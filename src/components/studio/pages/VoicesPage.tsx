...
function VoicePortrait({ image, name }: { image: string | null; name: string }) {
  ...
  return (
    <img
      src={image}
      alt={name}
      className="h-full w-full object-cover grayscale transition-all duration-700 ease-out group-hover:grayscale-0 group-hover:scale-105 relative z-0"
        ...
        <Container>
          <Reveal>
            ...
            <div className="hero-actions flex flex-wrap gap-4">
              ...
              <Link href="/pathway" className="btn-pill border border-inverse-border text-inverse-foreground hover:bg-inverse-foreground/10">Explore the Path</Link>
            </div>
          </Reveal>
        </Container>
      </section>
        ...
        <Container>
          <Reveal>
               ...
               {VOICES_LIST.map((voice, i) => (
                  <div key={i} className="group bg-card rounded-2xl overflow-hidden ring-1 ring-border/40 flex flex-col h-full hover:ring-primary/30 transition-all duration-300 cursor-default">
                     <div className="aspect-[4/5] bg-section w-full relative overflow-hidden mb-0">
                        <div className="absolute inset-0 bg-primary/5 group-hover:bg-transparent transition-colors duration-500 z-10 mix-blend-multiply pointer-events-none"></div>
                        <VoicePortrait image={voice.image} name={voice.name} />
                     </div>
            ...
            <p className="text-center text-[0.98rem] text-ink-soft italic font-serif-display text-xl w-full">
              This list is curated. Each name represents a deliberate addition to the conversation.
            </p>
          </Reveal>
        </Container>
      </section>
