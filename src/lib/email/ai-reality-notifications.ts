...
export async function sendMapResultEmail(input: {
  ...
  const gapList =
    gaps.length > 0
      ? `<ul style="font-size: 15px; line-height: 1.6; padding-left: 20px;">${gaps
          ...
          .join("")}</ul>`
      : `<p style="font-size: 15px; line-height: 1.6;">Nothing sharp surfaced — your footing looks solid across the board.</p>`;
  ...
  const { error } = await resend.emails.send({
    ...
    html: `${SHELL_OPEN}
      ...
      <p style="font-size: 16px; line-height: 1.6;">You answered six honest questions about where AI actually stands in your organization. Here's what surfaced, sharpest first:</p>
      ${gapList}
      <p style="font-size: 16px; line-height: 1.6;">Wherever you landed, the first move is the same one: <strong>Safety</strong>. Not a policy nobody reads — boundaries your people can actually train against.</p>
